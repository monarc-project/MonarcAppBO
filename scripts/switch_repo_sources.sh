...
MODE="${1:-}"
SERVICE_NAME="monarcboapp"

CORE_PACKAGE="monarc/core"
APP_PACKAGE="monarc/backoffice"

CORE_REPOSITORY_KEY="repositories.monarc-core"
APP_REPOSITORY_KEY="repositories.monarc-backoffice"

CORE_PATH="/var/www/html/zm-core"
APP_PATH="/var/www/html/zm-backoffice"
...
APP_NG_NAME="ng_backoffice"
APP_NG_REMOTE_URL="https://github.com/monarc-project/ng-backoffice.git"
APP_NG_LOCAL_TARGET="../../ng-backoffice"
APP_NG_CONTAINER_PATH="/var/www/html/ng-backoffice"

usage() {
    cat <<'EOF'
Switch BackOffice dependencies between local sibling repos and remote packages.
...
print_status() {
    ...
    if docker compose -f "${COMPOSE_FILE}" exec -T "${SERVICE_NAME}" sh -lc "[ -L /var/www/html/monarc/vendor/monarc/core ] && [ -L /var/www/html/monarc/vendor/monarc/backoffice ]"; then
        php_mode="local"
    fi
